...
class UNet(nn.Module):
    def __init__(
        ...
        self.conv3_1 = Block(n_filters[4], n_filters[3], 2, bias=bias)
        self.conv2_2 = Block(n_filters[3], n_filters[2], 2, bias=bias)
        self.conv1_3 = Block(n_filters[2], n_filters[1], 2, bias=bias)
        self.conv0_4 = Block(n_filters[1], n_filters[0], 2, bias=bias)
        if down =='pool':
        ...
        if up == 'deconv':
            self.up4 = nn.ConvTranspose2d(n_filters[4], n_filters[3], 2, 2, bias=bias)
            self.up3 = nn.ConvTranspose2d(n_filters[3], n_filters[2], 2, 2, bias=bias)
            self.up2 = nn.ConvTranspose2d(n_filters[2], n_filters[1], 2, 2, bias=bias)
            self.up1 = nn.ConvTranspose2d(n_filters[1], n_filters[0], 2, 2, bias=bias)
        else:
            self.up4 = nn.Sequential(
                nn.Upsample(scale_factor=2,mode='bilinear',align_corners=True),
                nn.Conv2d(n_filters[4],n_filters[3],1,bias=bias)
            )
            self.up3 = nn.Sequential(
                nn.Upsample(scale_factor=2,mode='bilinear',align_corners=True),
                nn.Conv2d(n_filters[3],n_filters[2],1,bias=bias)
            )
            self.up2 = nn.Sequential(
                nn.Upsample(scale_factor=2,mode='bilinear',align_corners=True),
                nn.Conv2d(n_filters[2],n_filters[1],1,bias=bias)
            )
            self.up1 = nn.Sequential(
                nn.Upsample(scale_factor=2,mode='bilinear',align_corners=True),
                nn.Conv2d(n_filters[1],n_filters[0],1,bias=bias)
            )

        if deep_supervision:
            self.out1 = nn.Conv2d(n_filters[0], num_classes, 1, bias=bias)
            self.out2 = nn.Conv2d(n_filters[1], num_classes, 1, bias=bias)
            self.out3 = nn.Conv2d(n_filters[2], num_classes, 1, bias=bias)
            self.out4 = nn.Conv2d(n_filters[3], num_classes, 1, bias=bias)
        else:
